 ...
 */
class Newspack_RSS_Enhancements {
	...
	public static function get_feed_settings( $feed_post = null ) {
		...
		if ( ! $feed_post ) {
			...
			$feed_post = get_page_by_path( sanitize_text_field( $query_feed ), OBJECT, self::FEED_CPT );
			if ( ! $feed_post ) {
				return false;
			}
		}
	...
	public static function add_extra_tags() {
		...
		if ( $settings['use_updated_tags'] ) {
			?>
			<updated><?php echo get_the_modified_date( 'Y-m-d\TH:i:s' ); ?></updated>
			<?php
		}
		...
		if ( $settings['use_media_tags'] ) {
			...
			if ( $thumbnail_id ) {
				...
				if ( $thumbnail_data ) {
					...
					<media:thumbnail url="<?php echo esc_attr( $thumbnail_data[0] ); ?>" width="<?php echo esc_attr( $thumbnail_data[1] ); ?>" height="<?php echo esc_attr( $thumbnail_data[2] ); ?>" />
					<media:content type="image/jpeg" url="<?php echo esc_attr( $thumbnail_data[0] ); ?>" />
					<?php
				}
			}
		}
